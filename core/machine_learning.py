...
class SVMDetector:
    def __init__(self, base_dir):
        ...
        if os.path.exists(model_path) and os.path.exists(scaler_path) and os.path.exists(metadata_path):
            ...
            if os.path.exists(imputer_path):
                ...
                print("⚠ imputer.pkl tidak ditemukan — NaN akan diisi median dari data saat ini (fallback)")
        else:
    ...
    def run_detection_pipeline(self, extracted_cells, bounding_boxes_sep,
        ...
        X = df_features[selected_features].replace([np.inf, -np.inf], np.nan)
        ...
        if self.imputer is not None:
            ...
            # Fallback: kalau imputer.pkl tidak ada (model lama)
